...
export default function Navbar() {
  ...
  return (
    <div>
      {/* Mobile */}
      <div className="lg:hidden fixed flex flex-row justify-between bg-white items-center mb-10 md:mb-10 lg:mb-0 px-4 py-2 md:px-8 md:py-6 left-0 w-full z-10">
        <Link href="/" className="lg:hidden items-center relative">
                ...
                <div>
                  <div className="fixed h-full w-screen right-0 translate-y-0 transition-all top-0 py-4">
                    <div className="flex bg-white flex-col absolute inset-x-0 top-0 h-80 p-8 gap-5 z-100 w-full">
                      ...
                      <Btn name="Let's Talk" link="mailto:isrealitycreations@gmail.com"/>
        ...
        <div className="hidden md:hidden lg:flex flex-row gap-80 2xl:gap-200 items-center justify-between">
          <nav className="flex flex-row gap-10 font-medium">
            ...
            <Link
              href="/"
              className={`${
                pathname === '/' ? 'text-black font-semibold text-2xl 2xl:text-3xl' : 'text-[#646464] hover:text-primary text-2xl 2xl:text-3xl'
              }`}
            >
              HOME
            </Link>
            <Link
              href="/about"
              className={`${
                pathname === '/about' ? 'text-black font-semibold text-2xl 2xl:text-3xl' : 'text-[#646464] hover:text-primary text-2xl 2xl:text-3xl'
              }`}
            >
              ABOUT 
            </Link>
            <Link
              href="/works"
              className={`${
                pathname === '/works' ? 'text-black font-semibold text-2xl 2xl:text-3xl' : 'text-[#646464] hover:text-primary text-2xl 2xl:text-3xl'
              }`}
            >
              MY WORKS
            </Link>
